...
class ZebpayAPIOrderBookDataSource(OrderBookTrackerDataSource):
    ...
    async def listen_for_subscriptions(self):
        ...
        # (order book is the "realtime" hot path; trades can be a touch slower) so one
    ...
    async def _poll_loop(self, fetch_coro, interval: float, label: str):
        while True:
            try:
                ...
                self.logger().exception(f"Unexpected error in Zebpay {label} polling loop. Retrying in 5 s …")
