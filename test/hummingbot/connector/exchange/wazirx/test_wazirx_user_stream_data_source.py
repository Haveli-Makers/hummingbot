...
class WazirxUserStreamDataSourceUnitTests(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_listen_for_user_stream_successful(self, mock_api):
        ...
        # Let it run for a short time to fetch data
    ...
    @aioresponses()
    @patch("hummingbot.connector.exchange.wazirx.wazirx_api_user_stream_data_source.WazirxAPIUserStreamDataSource._sleep")
    async def test_listen_for_user_stream_iter_message_throws_exception(self, mock_api, mock_sleep):
        ...
        # Mock sleep to raise CancelledError to stop the loop
    ...
    async def test_listen_for_user_stream_connection_failed(self, mock_api):
        ...
        # Should not have queued any messages due to error
        self.assertEqual(0, msg_queue.qsize())

    async def test_listen_for_user_stream_order_update(self):
        # Test processing order update events
        order_update_event = {
            "event": "orderUpdate",
    ...
    async def test_listen_for_user_stream_balance_update(self):
        # Test processing balance update events
    ...
    async def test_listen_for_user_stream_unknown_event(self):
        # Test processing unknown events (should still be queued)
        unknown_event = {
            "event": "unknownEvent",
            "timestamp": 1640995200000,
            "data": "some data"
