...
class DexalotRateSourceTest(IsolatedAsyncioWrapperTestCase):
    ...
    async def test_get_prices_exception(self, mock_api):
        ...
        if hasattr(rate_source.get_prices, 'cache_clear'):
    ...
    async def test_get_bid_ask_prices_exception(self, mock_api):
        ...
        if hasattr(rate_source.get_bid_ask_prices, 'cache_clear'):
            rate_source.get_bid_ask_prices.cache_clear()
        with patch.object(rate_source._exchange, 'get_all_pairs_prices', side_effect=Exception("API error")):
    ...
    async def test_get_bid_ask_prices(self, mock_api):
        ...
        with patch("aiohttp.ClientSession.ws_connect", new_callable=AsyncMock) as ws_connect_mock:
            ...
            result_subscribe = {'data': [
                {'pair': 'ALOT/USDC', 'date': '2024-10-04T08:54:32.021Z', 'low': '9', 'high': '11',
                 'open': '0.56628', 'close': '0.5659', 'volume': '124062.5422952677657237',
                 'quote_volume': '70336.660027130678322247184899', 'change': '-0.0007'},
                {'pair': 'UNKNOWN/USDC', 'date': '2024-10-04T08:54:32.021Z', 'low': '1', 'high': '2',
                 'open': '1', 'close': '1', 'volume': '1', 'quote_volume': '1', 'change': '0'}], 'type': 'marketSnapShot'}
        ...
        expected_bid = Decimal("9")
        expected_ask = Decimal("11")
        ...
        self.assertEqual(expected_bid, bid_ask_prices[self.trading_pair]["bid"])
        self.assertEqual(expected_ask, bid_ask_prices[self.trading_pair]["ask"])
        self.assertEqual(expected_mid, bid_ask_prices[self.trading_pair]["mid"])
        self.assertEqual(expected_spread, bid_ask_prices[self.trading_pair]["spread"])
        self.assertEqual(expected_spread_pct, bid_ask_prices[self.trading_pair]["spread_pct"])

    @aioresponses()
    async def test_get_bid_ask_prices_invalid_data(self, mock_api):
        rate_source = DexalotRateSource()
        rate_source._ensure_exchange()

        invalid_records = [
            {'pair': 'ALOT/USDC', 'low': '0', 'high': '10'},
            {'pair': 'ALOT/USDC', 'low': '10', 'high': '5'},
            {'pair': 'ALOT/USDC', 'low': '', 'high': '10'},
            {'pair': 'ALOT/USDC', 'low': '10', 'high': None},
        ]

        with patch.object(rate_source._exchange, 'get_all_pairs_prices', return_value=invalid_records), \
             patch.object(rate_source._exchange, 'trading_pair_associated_to_exchange_symbol', return_value=self.trading_pair):
            bid_ask_prices = await rate_source.get_bid_ask_prices()
            self.assertEqual({}, bid_ask_prices)  # No valid pairs

    @aioresponses()
    async def test_get_bid_ask_prices_parsing_error(self, mock_api):
        rate_source = DexalotRateSource()
        rate_source._ensure_exchange()

        error_records = [
            {'pair': 'ALOT/USDC', 'low': 'invalid', 'high': '10'},
        ]

        with patch.object(rate_source._exchange, 'get_all_pairs_prices', return_value=error_records), \
             patch.object(rate_source._exchange, 'trading_pair_associated_to_exchange_symbol', return_value=self.trading_pair):
            bid_ask_prices = await rate_source.get_bid_ask_prices()
            self.assertEqual({}, bid_ask_prices)
